...
int main(int argc, char** argv) {
    ...
    seeta::ModelSetting FA_model;
    FA_model.append(ModelPath + "face_landmarker_pts5.csta");
    FA_model.set_device(seeta::ModelSetting::CPU);
    FA_model.set_id(device_id);

    // init
    seeta::FaceDetector fd(FD_model);
    seeta::FaceLandmarker fa(FA_model);

    int landmark_num = fa.number();
    ...
    while (1) {
        ...
        vector<vector<SeetaPointF>> landmarks;
        landmarks.resize(faces.size, vector<SeetaPointF>(landmark_num));
        ...
        for (int i = 0; i < faces.size; ++i)
        {
            ...
            fa.mark(image, face, landmarks[i].data());
        }
